...
class DataFabric:
    ...
    def search_string(self, column: str, search_term: str, exact_match: bool = False):
        """Search for a string within a specified column with constraints."""
        if self.data is None:
            raise ValueError("Data not loaded. Call load_csv first.")

        if exact_match:
            result = self.data[self.data[column] == search_term]
        else:
            result = self.data[self.data[column].str.contains(search_term, case=False, na=False)]
...
if __name__ == "__main__":
    ...
    search_results = fabric.search_string(column='Faction', search_term='Warrior', exact_match=False)
